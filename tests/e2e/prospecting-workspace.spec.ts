...
test('a read-only deployment opens the real calling workflow without exposing mutations', async ({ page }) => {
  ...
  await expect(page.getByText(/use Open live calling to start a real session in production/i)).toBeVisible()
  await expect(page.getByRole('link', { name: 'Open live calling' })).toHaveAttribute('href', `https://crm.savingkc.com/prospecting?campaign=${dialerCampaign.id}`)
  ...
  await expect(page.getByText('Current seller')).toBeVisible()

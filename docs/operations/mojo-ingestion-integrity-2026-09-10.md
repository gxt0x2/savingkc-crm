...
- A seven-day Central calendar replay replaces the independent end-of-day implementation. The importer requests each Central day explicitly using Mojo's `start_date`/`end_date` contract, extending to an older successful checkpoint when needed. Every requested day must succeed; malformed or wrong-day rows, conflicting identities, a 5,000-row daily safety limit, and a 100-day catch-up limit retain the checkpoint. Manual `mojo:eod -- --date YYYY-MM-DD --dry-run` performs provider reads only. Historical day recovery never advances the live cursor.
...
- When several dated recordings fall within 90 minutes of the activity, intake attaches the nearest timestamp and, on a tie, the longest duration. Other in-window recording ids are kept as secondary evidence and can still match a later call. Undated and out-of-window recordings remain unmatched, including a sole undated contact recording. Matched calls use the recording timestamp. Activity grouping cannot cross Central dates. Current contact notes and unrelated current calendar events cannot be substituted for historical activity evidence.
...
Production verification caught two integration gaps after the initial repair: the private source route needed registration in the service-bearer proxy allowlist (#642), and Mojo ignores the old importer's `page` parameter. Its shipped client (`https://app71.mojosells.com/static/js/App.48fa2ac8.js`, inspected September 10) uses explicit `start_date` and `end_date` values formatted `MM/DD/YYYY`. Live requests confirmed that pages 1 and 2 returned the same 134 activities, while the requested September 3–10 range returned 136. Single-day requests returned only their requested dates, including an empty holiday. The importer now uses that verified date contract and fails closed when the provider ignores it.

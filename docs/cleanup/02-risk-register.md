...
| R-011 | Broad bearer-trust prefixes include sensitive API namespaces | High | Route handlers are guarded; enrichment and the six current `/api/workers/*` routes are now exact-listed while `/api/admin/`, `/api/ari/`, and `/api/cron/` still inherit namespace-level proxy trust | Continue replacing one reviewed namespace prefix at a time with exact routes | Re-add only the affected namespace prefix if an authenticated integration breaks |

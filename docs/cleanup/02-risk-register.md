...
| --- | --- | --- | --- | --- | --- |
| R-001 | CRM repo is public while GitHub secret-scanning has open public-leak alerts | Critical | `gxt0x2/savingkc-crm` is public; open alerts for Google API key, Supabase service key, and Twilio account SID | Confirm rotation status; rotate/restrict affected credentials; resolve alerts only after rotation | Revoke/rotate credentials; temporarily make repo private if needed |
| R-002 | Website has hardcoded browser keys in public/static code | High | `savingkc-website/public/partial-capture.js`; `savingkc-website/src/components/LeadForm.astro` | Restrict keys by domain/API; move config to build-time env where appropriate; review RLS | Rotate/restrict key; deploy previous Cloudflare/Vercel build if breakage occurs |

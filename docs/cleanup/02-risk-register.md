...
| R-011 | Broad bearer-trust prefixes include sensitive API namespaces | High | Enrichment and workers are exact-listed; ARI now requires a verified CRM session at both proxy and handler layers; `/api/admin/` and `/api/cron/` still inherit namespace-level proxy trust | Continue replacing one reviewed namespace prefix at a time with exact routes | Re-add only the affected namespace prefix if an authenticated integration breaks |
...
| R-030 | Gmail OAuth env vars are missing in Vercel | High | Vercel has Google Maps keys but no `GOOGLE_OAUTH_CLIENT_ID` or `GOOGLE_OAUTH_CLIENT_SECRET`; Gmail sync shows token refresh failures for the saved `ernest@savingkc.com` connection | Create or recover the Google Auth Platform web client, add both OAuth env vars to Vercel, redeploy, and reconnect Gmail | Remove the env vars and redeploy to disable Gmail sync without deleting saved Supabase token rows |

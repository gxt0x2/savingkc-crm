...
Final dispatch continuation: uncertain attempts block replacement intents and are visible as Review sending. Both sender capabilities are rechecked at dispatch. The worker refreshes one existing active sender domain after 12 hours so the 24-hour freshness guard does not silently prevent a day-8 follow-up; manual pauses stay paused, and provider failures hold sending. Twelve dispatch database tests pass (89 database cases overall across the verified suites), with 108 Email unit tests. Runtime proof of actual delivery remains pending owner inputs.

Budget-model evaluation: reducing reasoning allowed the free gpt-5-nano model to return structured output, but its callback draft claimed “I’ve got you down for a call” without any scheduling action. The fallback was rejected; no production model change or purchase occurred. That failure and related unsupported call commitments are now explicit policy-v2 regression cases, returned for human review with no draft body.

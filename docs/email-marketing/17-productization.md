...
- A Resend Email product API key pasted on Connections after `EMAIL_CREDENTIALS_KEY_V1` can be read by hosted Email routes. Named key `SavingKC Email CRM` exists off-chat. `EMAIL_CREDENTIALS_KEY_V1` is present on savingkc-crm Prod/Preview and `RESEND_API_KEY` is present Prod/Preview/Dev — **present-but-not-live** (no Email-route deploy). `RESEND_API_KEY` is Conversations, not Email. See [hosted-secrets contract](20-hosted-secrets-contract.md). Webhook secret for `POST /api/webhooks/email/resend` is **not created**. After-release URL is `https://crm.savingkc.com/api/webhooks/email/resend`. Do not create it from this VM.
...
- A funded Ari / AI Gateway path that this environment can authenticate. Ops reports ~$4.97 Free Credit; this VM retest is **FAIL** / `AI_NOT_CONNECTED` (no OIDC or gateway key). No real generation. Deterministic examples are not a paid model evaluation. AI paths stay fail-closed. See [Ari retest](21-ari-gateway-retest.md).

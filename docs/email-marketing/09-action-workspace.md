...
Keep Inbox, Campaigns and More in the white/red/grey theme. The inbox has a conversation list, selected conversation, and a Details column that starts expanded for each selected conversation. The column has distinct labeled icon tabs: Next step, Contact, Property, Calendar, Notes, and Ari’s Insights. Show one tab panel at a time; preserve unsaved notes and task entries between tabs. Arrow keys, Home and End move between tabs. On narrow screens Details becomes a fixed panel; Escape closes it and restores focus. Each panel scrolls independently beneath the visible tab strip. Long message history must not push the composer down the page.
...
## Compact workspace and reference cards

The September 12 density revision puts Email navigation and sending status on one row, keeps one compact practice/freshness strip, and removes duplicate Inbox and Details headings. At 1440 by 1000 the conversation starts near 269px rather than 406px with a notice present. The right-column tab strip includes its collapse control without a separate heading row.

Property uses an address/image card and two-column Beds, Baths, Square feet and Year built tiles. No photo or Street View is fabricated: the local preview explicitly reports imagery unavailable. Calendar uses a task form with editable title, attached Lead, fixed current task type/assignment/role, Chicago due time and optional notes. Save commits title and notes with the canonical callback activity; title projection is verified against the work item. Cancel restores saved title/notes and clears the proposed date. Call outcome is a separate disclosure. This remains an internal follow-up task, not a calendar invitation or general reassignment implementation.

## Default Next step and property valuation

Next step is the default tab. It owns the existing action/reply controls and CRM repair actions; those controls no longer appear above the message history. Calendar owns the scheduling form and highlights its context summary in light green. Ari’s Insights retains supporting conversation context and links to Next step instead of duplicating the action or prepared reply.

Calendar also lists the current linked Lead's open canonical work items in due-date order, with assignee, type, notes and Scheduled, Overdue, Held or Needs a date status. Completed/cancelled work and unrelated Leads stay out. Show up to 50 earliest open items, disclose truncation, and keep the list independently scrollable. Type and Assigned to dropdowns filter this list; they do not reassign tasks. CRM appointment tasks are labeled separately from Google Calendar bookings. External calendar events remain unavailable. The existing callback schedule form updates the same task represented in this list, rather than creating a duplicate.

Label that list Upcoming. Keep its Type and Assigned to controls collapsed behind a compact Filters button with an active-filter count. Scheduled task cards use pale yellow; the action summary remains green. The scheduling form lives under Scheduler, collapsed initially. Opening it from Next step expands it directly. Save and Cancel close it; manually collapsing it preserves unsaved input. Call outcome retains its own disclosure.

Property displays the canonical matched property’s saved `zestimate` when it is a positive finite number, with freshness explicitly unverified. No county assessment or invented value substitutes for Zillow data. The existing property JSON projection supplies the field when present in the actual canonical schema; the reduced local fixture has no valuation. Fabricated practice properties show Not available and do not link to a guessed real property. Real matched addresses get a Zillow address-search link, labeled Find property on Zillow. Exact Zillow property-page correlation and fresh provider retrieval remain unimplemented; an address-search link is not proof of a verified property page. The About the Zestimate link uses Zillow’s official explanation at https://www.zillow.com/zestimate/.

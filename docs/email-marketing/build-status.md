...
Updated: 2026-09-13 (ops-verified outreach DNS; live send still gated). **Connected local simulation; not a completed product or release candidate.**
...
Owner lock 2026-09-13 (purchase + ops-verify): Ernest/Robin purchased `talktosavingkc.com`, `savingkcteam.com` and `yourkchomebuyer.com` via Cloudflare Registrar. Nameservers are on Cloudflare. `savingkc.com` is untouched. Robin has now landed Resend domain add + Cloudflare DNS-only email records: `talktosavingkc.com` Resend verified (send+receive); `savingkcteam.com` and `yourkchomebuyer.com` send verified, receive enabled, Resend rechecking/partial. Record pattern: DKIM TXT `resend._domainkey`; MX `send` → `feedback-smtp.us-east-1.amazonses.com` p10; TXT `send` SPF amazonses; TXT `_dmarc` `p=none`; MX `@` → `inbound-smtp.us-east-1.amazonaws.com` p10. This code path does not invent product readiness from those writes. Do not buy more domains. Still no live customer send or production migrate without Ernest release auth. The existing Resend signup under `ernest@savingkc.com` has no product API key wired into Email. Ari credits remain unfunded.

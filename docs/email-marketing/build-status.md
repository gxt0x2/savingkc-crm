...
3. Local #3 software is packet-complete as far as it can go without live evidence (see [productization](17-productization.md) and [local #3 report](18-local-3-packet-complete.md)). Packets stay Partial. Outreach DNS is now ops-verified (partial on two domains) and recorded honestly in the intended-domain catalog with `sendingReady=false`. Remaining #3 live dependencies — Resend product API key + hosted secrets, funded Ari credits, Ernest release auth, verified Google Calendar, VAPID + device registration, and an owned response-line test — not more local scaffolding. Preserve everyday-language sales voice, verified facts, Lead → human-qualified Opportunity distinctions and weekday cadence. Live send stays gated. No customer send or prod migrate without release auth.

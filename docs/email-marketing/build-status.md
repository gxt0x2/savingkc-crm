...
| EM-009 | Partial: local all-marketing suppression, alias cancellation, opt-out fixtures, opaque public unsubscribe links, hosted-freeze `List-Unsubscribe` / `List-Unsubscribe-Post` headers, and a visible Unsubscribe anchor (no raw token URL in the text body) are implemented in code. Production preference-key deployment, a live Resend send, preference-center UX beyond one-click stop, and release/scope integration remain unfinished. Header wiring is not a production CLEAR. |
...
EM-009 local public preferences now include opaque versioned tokens (`em_preference_tokens`), HMAC key versions via `EMAIL_PREFERENCE_KEY_V*`, GET confirmation at `/email/unsubscribe/[token]` (no mutation), POST one-click stop at `/api/email/unsubscribe/[token]`, proxy bypass only for those exact method/path patterns, nullable `em_suppressions.created_by` for recipient-initiated stops, and honest `PUBLIC-UNSUBSCRIBE` audit with null actor. Links remain valid after issuer removal; forged/missing tokens fail closed; CRM history projection failures cannot roll back the unsubscribe. The local harness applies fifteen named Email migrations including `20260913160000_email_public_unsubscribe.sql`. No production migration, preference keys, live send, or customer email was deployed.

Verified locally for that preferences milestone: 75 database workflow cases (serial harness with `LC_ALL=C` / PostgreSQL 16), plus preference HTTP and proxy unit coverage for method/path containment. Remaining EM-009 gaps: deploy preference signing keys and `EMAIL_PUBLIC_ORIGIN`, confirm the Tax `em_senders` row is `ari@talktosavingkc.com`, preference center beyond one-click marketing stop, production schema apply, and release/auth checks.

Hosted Resend freeze (`freezeHostedEnvelope`) sets `List-Unsubscribe` to the one-click API URL and `List-Unsubscribe-Post: List-Unsubscribe=One-Click`. The outreach footer uses the counsel-v3 line with a human Unsubscribe link to the opaque preference URL and does not print that URL as bare text. Seller-outreach freeze accepts only `ari@talktosavingkc.com`. Every hosted program rejects the `savingkc.com` registrable domain. That corrects the earlier note that List-Unsubscribe wiring was still unfinished in code. It is not a production CLEAR, a live send, or proof that preference keys, `EMAIL_PUBLIC_ORIGIN`, or the Tax `em_senders` row are deployed.

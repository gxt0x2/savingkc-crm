...
3. Local #3 software is finished (see [productization](17-productization.md) and [local #3 report](18-local-3-packet-complete.md)). Packets stay Partial. Setup steps 4–6 now run the real AI / calendar / phone-push forms. Connections documents the hosted-secret contract: `SavingKC Email CRM` via Connections UI, `EMAIL_CREDENTIALS_KEY_V1`, `EMAIL_PREFERENCE_KEY_V*`, `EMAIL_RESEND_WEBHOOK_ENDPOINT_ID`, `POST /api/webhooks/email/resend`. Webhook secret is not created (needs public HTTPS after hosted Email routes deploy). Do not treat those secrets as sending-ready. Remaining live dependencies — funded Ari credits, Ernest release auth, verified Google Calendar, VAPID + devices, owned response-line test. Preserve everyday-language sales voice, verified facts, Lead → human-qualified Opportunity distinctions and weekday cadence. Live send stays gated. No customer send or prod migrate without release auth.
...
Still blocked for a controlled external test: wire the Email product Resend API key + hosted secrets; fund Ari/AI Gateway credits (AI paths stay fail-closed); Ernest release auth for live send. Also still unfinished: Google Calendar refresh + per-agent calendars; VAPID + per-user device registration; existing Twilio number ownership before TEL-TEST; hosted signed-in CRM-shell verification. Do not prod-migrate, deploy or send to customers from this VM.

...
- EM-004 local implementation is complete: an additive identity migration creates private parties, addresses, party-address and party-property links with workspace-scoped foreign keys and evidence. The pure resolver preserves local dots/plus aliases, normalizes only IDNA/lowercase domains, makes case-only variants review candidates and never selects from shared mailboxes. Focused checks now pass (**18 tests**). The migration passed in a disposable PostgreSQL 16 fixture with canonical CRM prerequisite tables; it inserted a confirmed identity link, enabled RLS and left zero browser-role table privileges. No SavingKC SQL was applied.
- EM-005 local implementation is complete: campaign review hashing binds the exact frozen snapshot/config/playbook reference; publication excludes ineligible rows, rejects duplicate active addresses and copied drafts omit execution/history. Focused campaign tests passed (**3 tests**). The additive campaign migration is written but its disposable database/RLS fixture remains pending. No campaign, enrollment, provider action or SavingKC SQL was run.
...
| EM-001 through EM-035 | EM-001 through EM-005 local work complete; EM-003–EM-005 migrations are unapplied to SavingKC. EM-006–EM-035 not started | Bounded dependency-ordered packets, allowed paths, required behavior and verification |
...
EM-004 is locally reviewable but both identity migrations remain unapplied to SavingKC. Before any production-target SQL, run the approved deployment preflight and direct permission/RLS and transaction cases for `20260912130000_email_configuration.sql` and `20260912140000_email_identity.sql`. Do not count local unit checks, metadata-only access or a disposable fixture as production verification. The next local dependency-ready packet is EM-005; controlled provider access becomes relevant to the early integrated path, while final production account/domain/audience/team/budget choices remain in the wizard.

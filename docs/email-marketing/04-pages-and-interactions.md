...
| THR-NOTE · Add CRM note | O/R/A with current thread access; `threadId,body` | Append one canonical Lead note with server actor/time and command receipt; replay returns the same note. Require a linked Lead. Available on stopped conversations for recordkeeping | T27,T31 |
...
| HAN-RESOLVE · Review held request | O/R/assigned A; `handoffId,ownerId,backupId,reason,positiveSellerInterest,requestedContact,factEvidence,contentRevision,controllerRevision` plus handoff revision | Re-review latest inbound for an unlinked held request; preserve prior evidence in audit and retry the same handoff/Lead bridge | T31,T35 |
| HAN-SCHEDULE · Save callback / book slot | A/O or server service under enabled scheduling policy; `handoffId,mode:task/calendar,startAt,timezone,contentRevision,title?,note?,phoneEvidence?,slotToken?` | Manual task or one verified internal calendar event, targeted owner alert and permitted post-booking email confirmation. Exact seller request/accepted slot required for auto-booking; no guessed time, dialing, SMS or guest invite | T33,T34,T53 |
...
The local action workspace in [revision 1.3](09-action-workspace.md) implements only manual callback tasks and completion, fenced by current handoff and content revisions. Calendar booking, general reopen and other outcomes remain unavailable.

...
Historical source baseline checked 2026-09-12. **Completion claims and old continuation instructions are superseded by [current build status](build-status.md). EM-003 remains partial; its older settings service still requires transactional authorization/readiness repair.** Reviewable checkpoint: `8190412c` on `codex/email-foundation-20260912`. It does not mark G0 fully passed, authorize SQL against production, or claim the email feature is implemented.
...
Use [build status](build-status.md), [the connected local milestone](recovery-milestone.md) and [verification evidence](local-verification.md). Do not restart completed local tests or infer provider readiness from the earlier baseline. The current implementation uses `src/lib/email/workflow/` and a disposable PostgreSQL/Next harness in `tests/email-local/`. Preserve the original CRM authorities and keep real dispatch disabled.

...
describe('prospecting workspace UI contract', () => {
  ...
  it('presents the internal Mojo and Launch Control workflows without fake predictive claims', () => {
    ...
    expect(workspace).toContain('/api/admin/stale-paused-dialer-session')
    expect(workspace).toContain('stale_paused_session_blocks_start')
    expect(dashboard).toContain('StalePausedDialerHardStopBanner')
    expect(dashboard).toContain('cannotStartNew={Boolean(hardStop && writesEnabled)}')
    expect(sessionSetup).toContain('cannotStartNew')
    expect(callingFloor).toContain('StalePausedDialerHardStopBanner')
    ...
    expect(studio).toContain('Put Tax 3+ or Deceased in the title—never both')
    expect(studio).toContain('Tax 3+ and Deceased lists are voice only')
    expect(studio).not.toContain('Casey ·')
    expect(audienceWorkbench).toContain('campaignName={campaignName}')
    expect(dashboard).toContain('prospectingDialerPickerLabel(campaign)')
    expect(dashboard).toContain('isProspectingDialerPickerCampaign(campaign)')
    expect(dashboard).not.toContain('{campaign.name} · {campaign.status}')

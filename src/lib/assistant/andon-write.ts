...
export async function listOpenAndons(db: Db, limit = 25) {
  ...
  return { action: 'list_open_andons' as const, writeScope: 'andon_only', andons }
}

export async function getAndon(db: Db, andonId: string) {
  return { action: 'get_andon' as const, writeScope: 'andon_only', andon: presentAndon(await readAndon(db, andonId)) }
}

...
function hashSalt(): string {
  return envText('PPC_TRACKING_SALT', 'CRON_SECRET', 'ADMIN_API_SECRET', 'DEPLOY_SECRET') ?? 'savingkc-ppc-v1'
}
...
function deviceType(userAgent: string | null, mobileHint: string | null): PpcRequestContext['devicePayload']['device_type'] {
  const ua = (userAgent ?? '').toLowerCase()
...
export function getPpcRequestContext(req: HeaderSource): PpcRequestContext {
  ...
  const rawIpRules = envList('PPC_INTERNAL_IPS', 'INTERNAL_TRAFFIC_IPS')
  if (ip && rawIpRules.some((rule) => ipMatchesRule(ip, rule))) internalReasons.push('ip_match')

  const ipHashRules = envList('PPC_INTERNAL_IP_HASHES', 'INTERNAL_TRAFFIC_IP_HASHES')
  if (hashMatches(ipHash, ipHashRules)) internalReasons.push('ip_hash_match')

  const uaHashRules = envList('PPC_INTERNAL_UA_HASHES', 'INTERNAL_TRAFFIC_UA_HASHES')

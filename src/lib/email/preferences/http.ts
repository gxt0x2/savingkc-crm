...
export function createPreferencePost(
  ...
  return async (request: Request, token: string) => {
    ...
    if (
      fields.getAll("List-Unsubscribe").length !== 1 ||
    ...
    try {
      ...
      return page(
        "<h1>You’re unsubscribed</h1><p>Marketing emails to this address have been stopped.</p>",
      );
    } catch {
      return page(

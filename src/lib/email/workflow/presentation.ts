...
export function nextWork(thread: PilotThread, asOf: string) {
  ...
  if (thread.callback_request && !thread.callback_request.reviewed) return thread.callback_request.testOnly ? 'Review test callback' : 'Review callback request'
  if (thread.callback_request?.reviewed) return 'Test callback reviewed'

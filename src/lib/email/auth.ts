...
export async function resolveEmailActor(
  request?: Request,
  ...
  const membership =
    await getEmailAdminDb().findActiveMembershipBySubject(subject)
...
export function requiredEmailCapability(
  command: EmailCommandId,
): 'configure' | 'pause' | null {
...
export function canRunEmailCommand(actor: EmailActor, command: EmailCommandId) {
  ...
  return (
    capability !== null &&
    actor.roles.some((role) => capabilities[capability].includes(role))
  )
}

export function requireEmailCommand(
  actor: EmailActor | null,
  command: EmailCommandId,
): asserts actor is EmailActor {
  if (!actor)
    throw new EmailAuthorizationError(
      'NO_SESSION',
      'A verified active Email workspace membership is required.',
    )
  if (!canRunEmailCommand(actor, command))
    throw new EmailAuthorizationError(
      'FORBIDDEN',
      'Your active Email workspace role cannot perform this action.',
    )
}

export class EmailAuthorizationError extends Error {
  constructor(
    public readonly code: 'NO_SESSION' | 'FORBIDDEN',
    message: string,
  ) {
    super(message)
  }

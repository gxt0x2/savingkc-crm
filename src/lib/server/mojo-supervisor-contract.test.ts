...
const extractor = fs.readFileSync('scripts/mojo-extract-session.mjs', 'utf8')
const performanceAdapterEntrypoint = fs.readFileSync('scripts/mojo-kpi-snapshot.mjs', 'utf8')
const performanceAdapter = fs.readFileSync('src/lib/server/mojo-kpi-provider.mjs', 'utf8')
...
describe('Mojo supervised recovery contract', () => {
  ...
  it('captures provider daily totals independently from filtered contact evidence', () => {
    ...
    expect(performanceAdapterEntrypoint).toContain("../src/lib/server/mojo-kpi-provider.mjs")
    ...
    expect(sync).not.toContain('syncMojoPerformanceSnapshot')
    ...
    expect(sync).toContain('/api/admin/mojo-source-batches')
  ...
  it('removes only tagged legacy Mojo schedules and keeps a recoverable backup', () => {
    ...
    expect(installer).toContain('MOJO_ENV_FILE')
    expect(installer).toContain('.env.production.local')
    expect(installer).toContain('--verify')
    expect(plist).toContain('__LOG_PATH__')
    expect(plist).not.toContain('/Users/ernestdodson')
    expect(sync).not.toContain('/Users/ernestdodson')
    expect(extractor).not.toContain('/Users/ernestdodson')
    expect(sessionHealth).not.toContain('/Users/ernestdodson')
  })

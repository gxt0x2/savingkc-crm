...
type UserWithKpiTargets = {
  name?: string | null
  roles?: { kpi_targets?: Partial<Record<string, number>> } | null
}
...
export async function generateAgentScorecard(
  ...
  try {
    ...
    const kpiTargets = (user as UserWithKpiTargets).roles?.kpi_targets || {}
    ...
    if (kpiTargets.daily_call_volume || kpiTargets.weekly_call_volume) {
      const target = period === 'daily'
        ? kpiTargets.daily_call_volume ?? 0
        : (kpiTargets.weekly_call_volume ?? 0) * 5
...
function aggregateStats(stats: AgentScorecardStats[]): AgentScorecardStats | null {

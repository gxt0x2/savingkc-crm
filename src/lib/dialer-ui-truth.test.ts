...
describe('Prospecting calling-floor truth contract', () => {
  ...
  it('keeps session controls explicit and the seller history bounded', () => {
    expect(sessionCommand).toContain("['Status', statusValue")
    expect(sessionCommand).toContain("['List', props.queueLabel")
    expect(sessionCommand).toContain("['Current', props.currentLabel")
    expect(sessionCommand).toContain("['Progress', `${props.currentIndex + 1} / ${props.queueSize}`")
    expect(sessionCommand).toContain('Stop this session?')
    expect(sessionCommand).not.toContain('bg-[#101827]')
    expect(sessionControls).toContain('Redial')
    expect(sessionControls).toContain('Hang up')
    expect(sessionControls).toContain("paused ? 'Resume' : 'Pause'")
    expect(sessionControls).toContain('Stop')
    expect(contextRail).toContain("{ id: 'notes', label: 'Notes'")
    expect(contextRail).toContain("{ id: 'details', label: 'Details'")
    expect(contextRail).toContain("{ id: 'follow_up', label: 'Follow-up'")
    expect(contextRail).toContain("{ id: 'appointment', label: 'Appointment'")
    expect(contextRail).toContain("{ id: 'mail', label: 'Mail'")

...
function AgentView({ ytd, appointments, report }: { ytd: YtdKpis | null; appointments: AppointmentStats | null; report: Report }) {
  return (
    <>
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard icon="phone_in_talk" label="Calls tracked" value={ytd?.ytd.calls ?? 0} detail={`${ytd?.ytd.dialTimeHrs.toFixed(1) ?? '0'} dialing hours`} tone="info" href="/reports/call-sms" />

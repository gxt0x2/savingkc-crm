            ...
            <table className="w-full min-w-[900px] border-collapse text-left">
              ...
              <tbody className="divide-y divide-[#e4e7ec]">
                {visible.map((workflow) => (
                  <tr key={workflow.id} onClick={() => setSelectedId(workflow.id)} className="cursor-pointer transition-colors hover:bg-[#fff8f8]" tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setSelectedId(workflow.id) }}>

...
export default async function ProspectingPage({ searchParams }: { searchParams: Promise<{ new?: string; campaign?: string; audience?: string; session_id?: string; lead_ids?: string; prospect_ids?: string; cohort?: string; preview_campaign?: string }> }) {
  const params = await searchParams
  const previewCampaignId = prospectingCampaignId(params.preview_campaign)
  const executionKey = params.session_id?.trim() || params.lead_ids?.trim() || params.prospect_ids?.trim() || params.cohort?.trim() || previewCampaignId

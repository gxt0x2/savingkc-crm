...
function previewTestReview(): CallRecordingItem {
  return {
    ...
    reviewedBy: null,
    transcript:
      'This is a preview-only test review. Use it to submit a framework, complete the scorecard, and verify the workflow.',
    transcriptActivityId: null,
    analysisSummary:
      'Test coaching opportunity: confirm motivation, timeline, decision makers, and a committed next step.',
    reviewWorkflow: {
      status: 'available',
      framework: null,
      submittedAt: null,
...
function leadName(lead: LeadRow | undefined, from: string): string {
  return (
    text(lead?.full_name) ||
    (from ? `Caller ${formatPhone(from) || from}` : 'Unknown caller')
  )
}
...
function findTranscriptFor(
  call: LeadActivityRow,
  transcriptRows: LeadActivityRow[],
): { id: string; text: string } | null {
  const sid = readRecordingSid(call.metadata)
  const direct = transcriptRows.find(
    (row) => readRecordingSid(row.metadata) === sid,
  )
  if (direct) {
  ...
  const fallback = transcriptRows.find((row) => {
    ...
    return (
      Number.isFinite(rowTime) && Math.abs(rowTime - callTime) <= 45 * 60 * 1000
    )
  })
...
function findAnalysisFor(
  ...
  return (
    ...
    compactTranscript(row.description || '') ||
    null
  )
}
...
function rowToItem(
  ...
  if (!recordingUrl || !recordingSid || durationSeconds <= 0 || !row.created_at)
    return null
  ...
  return {
    ...
    leadSource:
      lead?.source ||
      text(meta.lead_source) ||
      text(meta.context_source) ||
      null,
...
export async function GET(req: NextRequest) {
  ...
  const { data: callRows, error: callError } = await db
    ...
    .select(
      'id, lead_id, activity_type, description, metadata, created_at, agent',
    )
  ...
  const candidateCalls = ((callRows || []) as LeadActivityRow[]).filter(
    (row) => {
      const meta = record(row.metadata)
      if (isInternalTestRecording(meta)) return false
      return Boolean(
        playableRecordingUrl(meta) &&
          readRecordingDuration(meta) >= minDuration,
      )
    },
  )
  ...
  if (leadIds.length) {
    const { data, error } = await db
      .from('leads')
      .select(
        'id, full_name, phone, source, station, priority, property_address, city, state, classification, opportunity_score, created_at, updated_at, call_duration_seconds',
      )
      .in('id', leadIds)
    if (error) {
      return NextResponse.json(
  ...
  const transcriptRows = noteRows.filter(
    (row) => text(record(row.metadata).source) === 'whisper_transcription',
  )
  const analysisRows = noteRows.filter(
    (row) => text(record(row.metadata).source) === 'call_analysis',
  )
  ...
  if (process.env.VERCEL_ENV === 'preview')
    recordings.unshift(previewTestReview())

  const summary = buildRecordingSummary(
    recordings.map((item) => ({
      durationSeconds: item.durationSeconds,
      outcome: item.outcome as ReturnType<
        typeof readRecordingReview
      >['outcome'],
...
export async function PATCH(req: NextRequest) {
  ...
  if (
    (!activityId && !recordingSid) ||
    (!['submit', 'complete', 'retry_ai'].includes(action) &&
      !isRecordingReviewOutcome(outcome))
  ) {
    return NextResponse.json(
  ...
  let activityQuery = db
    ...
    .eq('activity_type', 'call')
    .limit(1)
  activityQuery = activityId
    ? activityQuery.eq('id', activityId)
    : activityQuery.eq('metadata->>recordingSid', recordingSid)
  const { data: activity, error: activityError } =
    await activityQuery.maybeSingle()
  ...
  if (!activity) {
    return NextResponse.json(
      { error: 'Recording activity not found' },
  ...
  if (action === 'retry_ai') {
    ...
    if (existing.status !== 'submitted') {
      return NextResponse.json(
        { error: 'Only submitted calls can be rescored' },
        { status: 409, headers: NO_STORE_HEADERS },
      )
    }
    if (existing.assignedReviewer !== email && !(await isCurrentUserAdmin())) {
      return NextResponse.json(
        { error: 'This review is assigned to another reviewer' },
  ...
  if (action === 'submit' || action === 'complete') {
    ...
    if (!framework)
      return NextResponse.json(
        { error: 'Select a valid review framework' },
        { status: 400, headers: NO_STORE_HEADERS },
      )
    const existing = readCallReviewWorkflow(activityRow.metadata)
    ...
    if (action === 'submit') {
      ...
      if (!isCallReviewer(assignedReviewer))
        return NextResponse.json(
          { error: 'Select a valid reviewer' },
          { status: 400, headers: NO_STORE_HEADERS },
        )
      const allowedTags = new Set<string>(CALL_REVIEW_TAGS)
      const tags = Array.isArray(body?.tags)
        ? body.tags.map(text).filter((tag) => allowedTags.has(tag))
        : []
      ...
      const reviewerName =
        CALL_REVIEWERS.find((reviewer) => reviewer.email === assignedReviewer)
          ?.name || assignedReviewer
      description = `Call submitted to ${reviewerName} for review — ${framework.label}`
    } else {
      if (existing.status !== 'submitted')
        return NextResponse.json(
          { error: 'Submit the call before completing its review' },
          { status: 409, headers: NO_STORE_HEADERS },
        )
      if (
        existing.assignedReviewer !== email &&
      ...
      const itemIds = framework.sections.flatMap((section) =>
      ...
      const aiCorrections = Object.entries(existing.aiAnswers).flatMap(
        ([id, assessment]) => (assessment.score === answers[id] ? [] : [id]),
      )
      const voiceoverPath = text(body?.voiceoverPath)
      const voiceoverMimeType = text(body?.voiceoverMimeType)
      if (
        voiceoverPath &&
        !voiceoverPath.startsWith(`call-review-voiceovers/${activityRow.id}/`)
      ) {
        return NextResponse.json(
          { error: 'Invalid coaching voiceover attachment' },
          { status: 400, headers: NO_STORE_HEADERS },
        )
      }
      updatedMetadata = mergeCallReviewWorkflow(activityRow.metadata, {
    ...
    const { error: workflowUpdateError } = await db
      .from('lead_activities')
      .update({ metadata: updatedMetadata })
    ...
    if (activityRow.lead_id) {
      await db.from('lead_activities').insert({
        ...
        metadata: {
          ...
          framework: framework.id,
          assigned_reviewer:
            readCallReviewWorkflow(updatedMetadata).assignedReviewer,
        },
      })
    }
    return NextResponse.json(
  ...
  const updatedMetadata = mergeRecordingReviewMetadata(
    (activity as LeadActivityRow).metadata,
    {

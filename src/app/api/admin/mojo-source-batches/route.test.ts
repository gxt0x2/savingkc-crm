...
const payload = { version: 'source-receipts-v1', since: '2026-09-03T05:00:00Z', to: '2026-09-10', activities: [[1, 11, 'Casey', '09/10/2026 10:00 AM', { group_name: 'Do Not Call' }]], recordings: [] }
const id = createHash('sha256').update(serializeMojoSource(payload)).digest('hex')
function request(method: string, body?: unknown) { return new Request('https://crm.savingkc.com/api/admin/mojo-source-batches', { method, ...(body ? { body: JSON.stringify(body) } : {}) }) as never }
describe('Mojo private source archive', () => {
  ...
  it('archives the exact source with an immutable duplicate policy', async () => {
    ...
    expect(mocks.upsert).toHaveBeenCalledWith({ id, payload, runtime_version: 'source-receipts-v1' }, { onConflict: 'id', ignoreDuplicates: true })
  })

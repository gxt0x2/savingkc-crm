...
export async function GET(request: NextRequest) {
  ...
  const { data: leadData, error: leadError } = await db
    .from('leads')
    .select('id, full_name, phone, email, property_address, city, source, station, priority, assigned_agent, opportunity_score, is_favorite, created_at, is_parked')
    .eq('is_parked', false)
    .neq('station', 'dead')
    .order('created_at', { ascending: false })
    .limit(5000)
  ...
  const [appointmentsResult, enhancedDealsResult, offersResult, buyersResult, revenueResult, expensesResult] = await Promise.all([
  ...
  const periodLeadIds = new Set(cohortLeads.map((lead) => lead.id))
  ...
  const periodActivities = activityResult.rows.filter((activity) =>
    periodLeadIds.has(activity.lead_id ?? '') && withinPeriod(activity.created_at, since, until),
  )

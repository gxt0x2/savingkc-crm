...
export function MobileContactsList({ items, selectedIds, onToggle, onOpen, onCall, onReviewPrimary, showScore = false }: { items: ContactWorkspaceRow[]; selectedIds: Set<string>; onToggle: (id: string) => void; onOpen: (id: string) => void; onCall: (row: ContactWorkspaceRow) => void; onReviewPrimary?: (row: ContactWorkspaceRow) => void; showScore?: boolean }) {
  return <div className="mt-3 space-y-2" aria-label="Contacts">{items.map((row) => {
    ...
    const nextAction = onReviewPrimary ? 'Primary action required' : row.primaryNextAction?.title || row.nextActivity?.label || (row.hubEnriched ? 'Define next action' : 'Loading next action…')
    ...
    return <article key={row.id} className="crm-panel rounded-2xl p-3.5">
      ...
      <div className="mt-2 flex items-center justify-between gap-2 text-xs"><span className="truncate text-[var(--crm-text-muted)]">{contactPipelineStatusLabel(row)}</span><span className="flex shrink-0 items-center gap-2">{showScore ? <span className="rounded-full bg-[var(--crm-brand-soft)] px-2 py-0.5 font-black text-[var(--crm-brand)]">{row.score} / 100</span> : null}<span className="text-[var(--crm-text-dim)]">{relativeDate(row.lastActivityAt)}</span></span></div>
      <button type="button" onClick={() => onReviewPrimary ? onReviewPrimary(row) : onOpen(row.id)} className={`mt-3 flex min-h-11 w-full items-center gap-2 rounded-xl border px-3 text-left text-sm font-bold ${row.primaryNextAction?.overdue ? 'border-[var(--crm-danger-border)] bg-[var(--crm-danger-soft)] text-[var(--crm-danger)]' : 'border-[var(--crm-action-border)] bg-[var(--crm-action-soft)] text-[var(--crm-action)]'}`}><Icon name={onReviewPrimary ? 'rule' : row.primaryNextAction?.overdue ? 'error' : 'schedule'} className="shrink-0" /><span className="min-w-0 flex-1 truncate">{nextAction}</span><Icon name="chevron_right" className="shrink-0" /></button>
      <div className="mt-2 grid grid-cols-2 gap-2"><button type="button" disabled={!row.phone} onClick={() => onCall(row)} className="crm-secondary-button flex min-h-10 items-center justify-center gap-2 rounded-xl text-sm font-bold disabled:opacity-40"><Icon name="call" />Call</button>{row.phone ? <Link href={`/conversations?lead=${row.id}&compose=sms`} className="crm-secondary-button flex min-h-10 items-center justify-center gap-2 rounded-xl text-sm font-bold"><Icon name="sms" />Text</Link> : <button type="button" disabled className="crm-secondary-button flex min-h-10 items-center justify-center gap-2 rounded-xl text-sm font-bold opacity-40"><Icon name="sms" />Text</button>}</div>
    </article>
  })}</div>
}

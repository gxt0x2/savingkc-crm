...
export function AcquisitionsMetricsDashboard({ report }: { report: OperatingReport }) {
  ...
  const cards = [
    ...
    { icon: 'verified', label: 'Opportunities', value: report.acquisitions.qualified, detail: `${percent(report.acquisitions.qualified, report.acquisitions.total)} opportunity rate`, tone: 'blue' as const, href: '/contacts?min_stage=qualified', series: report.trends.qualified, goal: scaledGoal(report.goals.weeklyQualified, report, 'weekly') },
...
function QualificationStatus({ report }: { report: OperatingReport }) {
  ...
  return <div className="grid min-h-[190px] gap-4 p-3 sm:grid-cols-[0.82fr_1.18fr]"><div><div className="grid grid-cols-3 gap-2"><MiniKpi label="Active" value={active} tone="green" /><MiniKpi label="Opportunities" value={report.acquisitions.qualified} tone="blue" /><MiniKpi label="Not leads" value={report.acquisitions.notLeads} tone="coral" /></div><p className="mt-3 text-[8px] font-bold uppercase text-[var(--crm-text-muted)]">Not-lead reasons</p>{reasons.length === 0 ? <p className="mt-4 text-center text-[9px] text-[var(--crm-text-muted)]">No not-lead outcomes recorded</p> : <div className="mt-1 divide-y divide-[var(--crm-border)]">{reasons.map((row) => <DataRow key={row.reason} label={row.reason} value={row.count} />)}</div>}</div><div><p className="text-[8px] font-bold uppercase text-[var(--crm-text-muted)]">Not leads by source</p>{bySource.length === 0 ? <div className="mt-10 text-center text-[9px] text-[var(--crm-text-muted)]">No source-level not-lead records</div> : <div className="mt-2 space-y-3">{bySource.map((row, index) => <BarRow key={row.source} label={formatLeadSource(row.source)} value={row.count} max={Math.max(...bySource.map((item) => item.count), 1)} color={SOURCE_COLORS[index]} />)}</div>}</div></div>
}
...
function RevenueLift({ report }: { report: OperatingReport }) {
  ...
  const rows = [
    ['Lead-to-opportunity rate', model.currentRates.qualification, model.optimizedRates.qualification],

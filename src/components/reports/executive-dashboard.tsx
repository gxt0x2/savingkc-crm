    ...
    <DashboardPanel title="Marketing" period="Selected period" tone="violet" href="/reports/marketing">
      <div className="grid h-full min-h-0 grid-rows-[78px_1fr] divide-y divide-[var(--crm-border)]">
        <div className="grid grid-cols-2 divide-x divide-[var(--crm-border)]">
          ...
          <div className="flex flex-col items-center justify-center"><strong className="text-2xl font-black text-[var(--crm-info)]">{attributedQualified}</strong><span className="text-[9px] font-bold text-[var(--crm-text-muted)]">Opportunities</span></div>
        </div>
        <div className="min-w-0 px-3 py-2"><div className="grid grid-cols-[1fr_44px_64px] gap-1 border-b border-[var(--crm-border)] pb-1 text-[8px] font-bold uppercase text-[var(--crm-text-muted)]"><span>Lead source</span><span className="text-right">Leads</span><span className="text-right">Opps</span></div>{sources.map((source) => <Link href="/reports/acquisitions" key={source.key} className="grid grid-cols-[1fr_44px_64px] gap-1 border-b border-[var(--crm-border)] py-1.5 text-[9px] hover:bg-[var(--crm-surface-subtle)]"><strong className="truncate">{source.label}</strong><span className="text-right">{source.leads}</span><span className="text-right font-bold text-[var(--crm-info)]">{source.qualified}</span></Link>)}
        </div>
      </div>
    </DashboardPanel>
  )

...
function DashboardView({ report }: { report: OperatingReport }) {
  const cards = [
    ...
    { icon: 'route', label: 'Pipeline offer value', value: report.core.pipelineOfferValue == null ? 'Not recorded' : money(report.core.pipelineOfferValue), numericValue: report.core.pipelineOfferValue, detail: 'Recorded seller offers on active leads', tone: 'violet' as const, href: '/reports/acquisitions', series: report.trends.underContract },
    ...
    <>
      <NumberedPanel number="1" title="Core acquisition metrics" hint="Lead-to-contract execution">
        <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <MetricCard icon="schedule" label="Speed to lead" value={minutes(report.acquisitions.averageSpeedToLeadMinutes)} numericValue={report.acquisitions.averageSpeedToLeadMinutes} detail="First recorded outbound action" tone="blue" href="/reports/call-sms" series={report.trends.calls} />
          <MetricCard icon="event_available" label="Appointments" value={available(report.availability.appointments, report.acquisitions.appointmentsRecorded)} numericValue={report.acquisitions.appointmentsRecorded} detail={`${nullablePercent(report.acquisitions.appointmentShowRate)} recorded show rate`} tone="violet" href="/calendar?department=acquisitions" series={report.trends.appointments} goal={scaledGoal(report.goals.weeklyAppointments, report, 'weekly')} />
          ...
          <MetricCard icon="mark_chat_unread" label="Needs reply" value={report.acquisitions.attention.needsReply} numericValue={report.acquisitions.attention.needsReply} detail="Current unresolved attention" tone="red" href="/conversations?reply=needs_reply" series={report.trends.sms} />
        </div>
      </NumberedPanel>
    ...
    <>
      <NumberedPanel number="1" title="Core marketing metrics" hint="CRM-attributed outcomes">
        <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard icon="campaign" label="Lead sources" value={report.marketing.sources.length} numericValue={report.marketing.sources.length} detail="Sources with recorded leads" tone="violet" href="/marketing" series={report.trends.leads} />
          ...
          <MetricCard icon="verified" label="Qualified" value={report.core.qualified} numericValue={report.core.qualified} detail={`${percent(report.core.qualified, report.core.leads)} qualification rate`} tone="blue" href="/contacts?min_stage=qualified" series={report.trends.qualified} goal={scaledGoal(report.goals.weeklyQualified, report, 'weekly')} />
          <MetricCard icon="trophy" label="Top source" value={top ? formatLeadSource(top.source) : 'No data'} numericValue={top?.leads ?? null} detail={top ? `${top.leads} recorded leads` : 'No lead-source records'} tone="green" href="/marketing" series={report.trends.leads} />
        </div>
      </NumberedPanel>
    ...
    <>
      ...
      <NumberedPanel number="1" title="Core disposition metrics" hint="Recorded in the selected period">
        <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-8">
          <MetricCard icon="forum" label="Active deals" value={available(report.availability.dispositions, report.dispositions.activeDeals)} numericValue={report.dispositions.activeDeals} detail="Not closed or dead" tone="blue" href="/dispo/pipeline" series={report.trends.underContract} />
          <MetricCard icon="sell" label="Offers / property" value={available(report.availability.offers, nullableDecimal(report.dispositions.offersPerProperty))} numericValue={report.dispositions.offersPerProperty} detail={`${report.dispositions.offers} recorded offers`} tone="green" href="/dispo/offers" series={report.trends.offers} />
          <MetricCard icon="speed" label="Days to buyer" value={available(report.availability.dispositions && report.availability.offers, nullableDays(report.dispositions.averageDaysToBuyer))} numericValue={report.dispositions.averageDaysToBuyer} detail="Entered dispo to accepted offer" tone="coral" href="/dispo/pipeline" series={report.trends.offers} />
          <MetricCard icon="payments" label="Assignment revenue" value={available(report.availability.dispositions, money(report.dispositions.assignmentRevenue))} numericValue={report.dispositions.assignmentRevenue} detail="Recorded closed assignment fees" tone="violet" href="/reports/finance" series={report.trends.revenue} />
          <MetricCard icon="attach_money" label="Avg assignment fee" value={available(report.availability.dispositions, nullableMoney(report.dispositions.averageAssignmentFee))} numericValue={report.dispositions.averageAssignmentFee} detail="Across recorded closed fees" tone="green" href="/reports/finance" series={report.trends.revenue} />
          <MetricCard icon="task_alt" label="Close rate" value={available(report.availability.dispositions, nullablePercent(report.dispositions.closeRate))} numericValue={report.dispositions.closeRate} detail="Closed / non-dead deals" tone="blue" href="/dispo/pipeline" series={report.trends.closings} />
          <MetricCard icon="groups" label="Active buyers" value={available(report.availability.buyers, report.dispositions.activeBuyers)} numericValue={report.dispositions.activeBuyers} detail="Buyer records marked active" tone="teal" href="/dispo/buyers" series={report.trends.offers} />
          <MetricCard icon="history" label="Debriefs due" value={available(report.availability.dispositions, report.dispositions.debriefOutstanding)} numericValue={report.dispositions.debriefOutstanding} detail="Closed loop incomplete" tone="red" href="/dispo/pipeline?closeout=due" series={report.trends.closings} />
        </div>
      </NumberedPanel>
    ...
    <>
      <NumberedPanel number="1" title="Core financial metrics" hint="Non-seed transactions only">
        <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <MetricCard icon="account_balance" label="Net" value={available(report.availability.finance, money(report.finance.netRevenue))} numericValue={report.finance.netRevenue} detail="Revenue less expenses" tone={report.finance.netRevenue >= 0 ? 'blue' : 'red'} href="/dispo/tc?view=reports" series={report.trends.revenue} />
          <MetricCard icon="percent" label="Profit margin" value={available(report.availability.finance, nullablePercent(report.finance.profitMargin))} numericValue={report.finance.profitMargin} detail="Net / gross revenue" tone="violet" href="/dispo/tc?view=reports" series={report.trends.revenue} />
          <MetricCard icon="price_check" label="Avg revenue" value={available(report.availability.finance, nullableMoney(report.finance.averageRevenuePerTransaction))} numericValue={report.finance.averageRevenuePerTransaction} detail="Per recorded revenue transaction" tone="teal" href="/dispo/tc?view=reports" series={report.trends.revenue} />
        </div>
      </NumberedPanel>
      <section className="grid gap-3 xl:grid-cols-[1.15fr_0.85fr]">
    ...
    <>
      <NumberedPanel number="1" title="Core communication metrics" hint="Recorded call and message events">
        <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-6">
          ...
          <MetricCard icon="phone_in_talk" label="Connected" value={report.communications.connectedCalls} numericValue={report.communications.connectedCalls} detail={`${nullablePercent(report.communications.callConnectionRate)} connection rate`} tone="green" href="/dialer?section=conversations" series={report.trends.calls} />
          <MetricCard icon="sms" label="SMS" value={report.communications.sms} numericValue={report.communications.sms} detail="All recorded SMS activity" tone="violet" href="/conversations?channel=sms" series={report.trends.sms} />
          <MetricCard icon="north_east" label="SMS sent" value={report.communications.outboundSms} numericValue={report.communications.outboundSms} detail="Recorded outbound messages" tone="teal" href="/conversations?channel=sms" series={report.trends.sms} />
          <MetricCard icon="south_west" label="SMS received" value={report.communications.inboundSms} numericValue={report.communications.inboundSms} detail={`${nullablePercent(report.communications.smsResponseRate)} inbound / outbound`} tone="amber" href="/conversations?channel=sms" series={report.trends.sms} />
          <MetricCard icon="mark_chat_unread" label="Needs reply" value={report.core.needsReply} numericValue={report.core.needsReply} detail="Current unresolved seller attention" tone="red" href="/conversations?reply=needs_reply" series={report.trends.sms} />
        </div>
      </NumberedPanel>
      <section className="grid gap-3 xl:grid-cols-[1.2fr_0.8fr]">
    ...
    <Link href={href} className={`crm-panel group flex min-h-[178px] flex-col overflow-hidden rounded-2xl border-t-[3px] p-3.5 transition-all hover:-translate-y-0.5 hover:shadow-[var(--crm-shadow-md)] ${palette.border}`} style={{ borderTopColor: palette.line }}>
      ...
      <div className="mt-auto pt-1.5">
        ...
        <div className="mt-1 flex items-center justify-between gap-2 text-[9px] font-bold">
          <span className={movement == null ? 'text-[var(--crm-text-dim)]' : movement >= 0 ? 'text-[var(--crm-success)]' : 'text-[var(--crm-danger)]'}>{movement == null ? 'No prior activity' : `${movement >= 0 ? '↑' : '↓'} ${Math.abs(movement)}% vs prior half`}</span>

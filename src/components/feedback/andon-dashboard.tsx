...
function useAndons(preset: RangePreset, customStart: string, customEnd: string) {
  return useQuery<{ items: AndonItem[]; total: number }>({
    queryKey: ['andon-log', preset, customStart, customEnd],
    queryFn: async () => {
      const query = rangeParams(preset, customStart, customEnd)
      const response = await fetch(`/api/feedback/log?${query}`, { cache: 'no-store' })
      if (!response.ok) throw new Error('The Andon dashboard could not load.')
      return response.json() as Promise<{ items: AndonItem[]; total: number }>
    },
    staleTime: 15_000,
...
export function AndonDashboard() {
  ...
  const filteredItems = useMemo(() => items.filter((item) => {
    ...
    if (search.trim()) {
      const haystack = [item.description, item.department, item.category, item.agent_name].join(' ').toLowerCase()
  ...
  const resolved = items.filter((item) => ['resolved', 'closed'].includes(item.status))
  ...
  const averageResolutionHours = resolved.length > 0
    ? Math.round(resolved.reduce((sum, item) => sum + Math.max(0, new Date(item.resolved_at ?? item.updated_at ?? item.created_at).getTime() - new Date(item.created_at).getTime()), 0) / resolved.length / 3_600_000)
    : null

  const kindCounts = Object.keys(ANDON_KIND_LABELS).map((kind) => ({
    kind: kind as AndonIssueKind,
    count: items.filter((item) => item.issue_kind === kind).length,
  }))
  ...
  async function updateItem(status: string, saveWhys = false) {
    if (!selected) return
    setSaving(true)
    setActionError('')
    try {
      const response = await fetch('/api/feedback/update-status', {
        ...
        body: JSON.stringify({
          ...
          five_whys: saveWhys && selected.source === 'feedback' ? draftWhys : undefined,
        }),
      })
      if (!response.ok) {
  ...
  return (
    <main className="mx-auto w-full max-w-[1720px] space-y-3 px-3 py-4 pb-24 sm:px-5 lg:px-6">
      <header className="crm-panel flex flex-col gap-4 rounded-2xl px-5 py-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <p className="crm-eyebrow">CEO Operating System · Continuous improvement</p>
          <h1 className="mt-1 text-[25px] font-black tracking-[-0.035em]">Andon system</h1>
          <p className="mt-0.5 max-w-3xl text-xs font-medium text-[var(--crm-text-muted)]">One live queue for process, system, and data issues - from first signal through root cause and verified resolution.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <label className="flex h-10 items-center gap-2 rounded-xl border border-[var(--crm-border)] bg-[var(--crm-surface)] px-3 shadow-sm">
            ...
            <span className="sr-only">Andon date range</span>
            <select aria-label="Andon date range" value={preset} onChange={(event) => setPreset(event.target.value as RangePreset)} className="bg-transparent text-xs font-black outline-none">
      ...
      {error ? <div role="alert" className="crm-panel flex items-center justify-between rounded-2xl border-[var(--crm-danger)]/30 px-5 py-4 text-sm font-bold text-[var(--crm-danger)]"><span>{error.message}</span><button type="button" onClick={() => void refetch()} className="crm-secondary-button rounded-lg px-3 py-2 text-xs">Retry</button></div> : null}
      ...
      <section aria-label="Andon operating metrics" className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <Metric icon="timer" label="Avg. resolution" value={averageResolutionHours == null ? 'Not recorded' : `${averageResolutionHours}h`} detail={`${resolved.length} resolved in range`} tone="green" />
      </section>

      <section className="grid gap-3 xl:grid-cols-[0.86fr_1.14fr]">
        <section className="crm-panel rounded-2xl p-4">
          <div className="mb-4"><p className="crm-eyebrow">Signal mix</p><h2 className="mt-1 text-base font-black">What is breaking good work?</h2></div>
          <div className="grid grid-cols-2 gap-2">
            {kindCounts.map(({ kind, count }) => { const tone = KIND_TONES[kind]; return <button key={kind} type="button" onClick={() => setKindFilter(kindFilter === kind ? 'all' : kind)} className={`rounded-xl border p-3 text-left transition-colors ${kindFilter === kind ? 'border-[var(--crm-brand)] ring-2 ring-[var(--crm-brand)]/10' : 'border-[var(--crm-border)]'}`}><span className="grid h-8 w-8 place-items-center rounded-lg" style={{ color: tone.color, background: tone.soft }}><Icon name={tone.icon} /></span><strong className="mt-2 block text-xl font-black">{count}</strong><span className="text-xs font-bold text-[var(--crm-text-muted)]">{ANDON_KIND_LABELS[kind]}</span></button> })}
          </div>
        </section>
        <section className="crm-panel rounded-2xl p-4">
          <div className="mb-4"><p className="crm-eyebrow">Work area concentration</p><h2 className="mt-1 text-base font-black">Where issues originate</h2></div>
          {departmentCounts.length > 0 ? <div className="space-y-3">{departmentCounts.map(([department, count]) => <div key={department} className="grid grid-cols-[120px_1fr_28px] items-center gap-3 text-xs"><strong className="truncate">{department}</strong><span className="h-2 overflow-hidden rounded-full bg-[var(--crm-surface-subtle)]"><span className="block h-full rounded-full bg-[var(--crm-info)]" style={{ width: `${Math.round(count / maxDepartment * 100)}%` }} /></span><strong className="text-right">{count}</strong></div>)}</div> : <EmptyState label="No Andons in this range" />}
        </section>
      </section>

      <section className="crm-panel overflow-hidden rounded-2xl">
        <header className="flex flex-col gap-3 border-b border-[var(--crm-border)] px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
          <div><p className="crm-eyebrow">Operating queue</p><h2 className="mt-1 text-base font-black">Andons requiring review</h2></div>
          <div className="flex flex-wrap gap-2">
            <label className="relative"><span className="sr-only">Search Andons</span><Icon name="search" className="pointer-events-none absolute left-3 top-2.5 text-[18px] text-[var(--crm-text-muted)]" /><input aria-label="Search Andons" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search Andons..." className="crm-field h-10 w-56 rounded-xl pl-9 pr-3 text-xs font-semibold" /></label>
            ...
            <select aria-label="Andon status filter" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="crm-field h-10 rounded-xl px-3 text-xs font-bold"><option value="active">Active</option><option value="all">All statuses</option>{ANDON_STATUSES.map((status) => <option key={status} value={status}>{status.replaceAll('_', ' ')}</option>)}</select>
          </div>
        </header>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px]">
            ...
            <tbody>{isLoading ? <tr><td colSpan={8} className="px-4 py-12 text-center text-sm font-semibold text-[var(--crm-text-muted)]">Loading live Andons…</td></tr> : filteredItems.length === 0 ? <tr><td colSpan={8}><EmptyState label="No Andons match these filters" /></td></tr> : filteredItems.map((item) => { const tone = KIND_TONES[item.issue_kind]; return <tr key={`${item.source}-${item.id}`} className="cursor-pointer border-t border-[var(--crm-border)] text-xs hover:bg-[var(--crm-surface-subtle)]" onDoubleClick={() => openItem(item)}><td className="max-w-[380px] px-4 py-3"><div className="flex items-start gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg" style={{ color: tone.color, background: tone.soft }}><Icon name={tone.icon} /></span><div className="min-w-0"><strong className="block">{ANDON_KIND_LABELS[item.issue_kind]}</strong><span className="mt-0.5 block truncate text-[var(--crm-text-muted)]">{item.description}</span></div></div></td><td className="px-4 py-3 font-bold">{item.department}</td><td className="px-4 py-3 text-[var(--crm-text-muted)]">{item.category}</td><td className="px-4 py-3"><ImpactBadge priority={item.priority} /></td><td className="px-4 py-3"><StatusBadge status={item.status} /></td><td className="px-4 py-3 text-[var(--crm-text-muted)]">{new Date(item.created_at).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</td><td className="px-4 py-3">{item.agent_name || 'System'}</td><td className="px-3 py-3"><button type="button" onClick={() => openItem(item)} aria-label={`Open Andon: ${item.description}`} className="crm-icon-button grid h-8 w-8 place-items-center rounded-lg"><Icon name="chevron_right" /></button></td></tr> })}</tbody>
          </table>
        </div>
      </section>

      {selected ? <div className="fixed inset-0 z-[100] bg-black/45" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedId(null) }}><aside role="dialog" aria-modal="true" aria-labelledby="andon-detail-title" className="absolute inset-y-0 right-0 w-full max-w-xl overflow-y-auto border-l border-[var(--crm-border)] bg-[var(--crm-surface)] p-5 shadow-2xl"><header className="flex items-start justify-between gap-4"><div><p className="crm-eyebrow">{ANDON_KIND_LABELS[selected.issue_kind]} · {selected.department}</p><h2 id="andon-detail-title" className="mt-1 text-xl font-black">{selected.category}</h2><p className="mt-1 text-xs text-[var(--crm-text-muted)]">Raised {new Date(selected.created_at).toLocaleString()} by {selected.agent_name || 'System'}</p></div><button type="button" onClick={() => setSelectedId(null)} aria-label="Close Andon details" className="crm-icon-button grid h-9 w-9 place-items-center rounded-lg"><Icon name="close" /></button></header><div className="mt-5 flex flex-wrap gap-2"><ImpactBadge priority={selected.priority} /><StatusBadge status={selected.status} />{selected.page_url ? <a href={selected.page_url} className="crm-secondary-button inline-flex items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-black"><Icon name="open_in_new" />Open reported page</a> : null}</div><section className="mt-5 rounded-xl border border-[var(--crm-border)] p-4"><h3 className="text-xs font-black uppercase tracking-wider">What happened</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{selected.description}</p></section><section className="mt-3 rounded-xl border border-[var(--crm-border)] p-4"><div className="flex items-center justify-between"><div><h3 className="text-xs font-black uppercase tracking-wider">Five Whys</h3><p className="mt-1 text-[11px] text-[var(--crm-text-muted)]">Complete the root-cause chain before resolution.</p></div><span className="rounded-full bg-[var(--crm-info-soft)] px-2 py-1 text-[10px] font-black text-[var(--crm-info)]">{draftWhys.filter(Boolean).length}/5</span></div><div className="mt-3 space-y-2">{draftWhys.map((why, index) => <label key={index} className="grid items-center gap-2 text-xs font-bold sm:grid-cols-[58px_1fr]"><span>Why {index + 1}</span><input disabled={selected.source === 'error_log'} value={why} onChange={(event) => setDraftWhys((current) => current.map((entry, entryIndex) => entryIndex === index ? event.target.value : entry))} className="crm-field h-10 rounded-lg px-3 text-xs font-medium disabled:opacity-60" placeholder="Root cause not recorded" /></label>)}</div>{selected.source === 'feedback' ? <button type="button" disabled={saving} onClick={() => void updateItem(selected.status, true)} className="crm-primary-button mt-3 w-full rounded-lg px-3 py-2 text-xs font-black">Save root-cause analysis</button> : null}</section><section className="mt-3 rounded-xl border border-[var(--crm-border)] p-4"><h3 className="text-xs font-black uppercase tracking-wider">Move through resolution</h3><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">{(selected.source === 'error_log' ? ['open', 'resolved'] : ANDON_STATUSES.filter((status) => status !== 'closed')).map((status) => <button key={status} type="button" disabled={saving || selected.status === status} onClick={() => void updateItem(status)} className={`rounded-lg border px-2 py-2 text-xs font-bold capitalize ${selected.status === status ? 'border-[var(--crm-info)] bg-[var(--crm-info-soft)] text-[var(--crm-info)]' : 'border-[var(--crm-border)] hover:border-[var(--crm-border-strong)] disabled:opacity-60'}`}>{status.replaceAll('_', ' ')}</button>)}</div></section>{actionError ? <div role="alert" className="mt-3 rounded-lg bg-[var(--crm-danger-soft)] px-3 py-2 text-xs font-bold text-[var(--crm-danger)]">{actionError}</div> : null}</aside></div> : null}
    </main>
  )
}

function Metric({ icon, label, value, detail, tone }: { icon: string; label: string; value: string | number; detail: string; tone: 'danger' | 'coral' | 'blue' | 'violet' | 'green' }) {
  const colors = { danger: ['var(--crm-danger)', 'var(--crm-danger-soft)'], coral: ['#f05a28', '#ffebe4'], blue: ['var(--crm-info)', 'var(--crm-info-soft)'], violet: ['var(--crm-violet)', 'var(--crm-violet-soft)'], green: ['var(--crm-success)', 'var(--crm-success-soft)'] }[tone]
...
function ImpactBadge({ priority }: { priority: string }) {
  const classes = priority === 'critical' ? 'bg-[var(--crm-danger-soft)] text-[var(--crm-danger)]' : priority === 'high' ? 'bg-[#ffebe4] text-[#d84315]' : priority === 'medium' ? 'bg-[var(--crm-info-soft)] text-[var(--crm-info)]' : 'bg-[var(--crm-surface-subtle)] text-[var(--crm-text-muted)]'
  return <span className={`inline-flex rounded-full px-2 py-1 text-[10px] font-black uppercase ${classes}`}>{priority}</span>
}

function StatusBadge({ status }: { status: string }) {
  const resolved = ['resolved', 'closed'].includes(status)
  return <span className={`inline-flex rounded-full px-2 py-1 text-[10px] font-black capitalize ${resolved ? 'bg-[var(--crm-success-soft)] text-[var(--crm-success)]' : status === 'open' ? 'bg-[var(--crm-danger-soft)] text-[var(--crm-danger)]' : 'bg-[var(--crm-violet-soft)] text-[var(--crm-violet)]'}`}>{status.replaceAll('_', ' ')}</span>
}

function EmptyState({ label }: { label: string }) {
  return <div className="grid min-h-36 place-items-center px-4 py-8 text-center"><div><Icon name="check_circle" className="text-[28px] text-[var(--crm-success)]" /><p className="mt-2 text-xs font-bold text-[var(--crm-text-muted)]">{label}</p></div></div>
}

...
describe('workspace navigation', () => {
  ...
  it('keeps a hovered menu open on first click and inherits the workspace theme', () => {
    ...
    const sections = screen.getByRole('navigation', { name: 'Prospecting sections' })
    expect(sections.closest('.crm-workspace-shell')).toBe(container.firstElementChild)
    expect(within(screen.getByRole('navigation', { name: 'CRM navigation' })).getByRole('navigation', { name: 'Prospecting sections' })).toBe(sections)
    expect(sections).not.toHaveClass('fixed')
  ...
  it('opens grouped navigation by click while the rail is collapsed', () => {
    ...
    expect(sections).toHaveClass('fixed')
    expect(within(screen.getByRole('navigation', { name: 'CRM navigation' })).queryByRole('navigation', { name: 'Reports sections' })).not.toBeInTheDocument()

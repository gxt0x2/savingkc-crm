...
import { CountyParcelAudienceEnroll } from '@/components/prospecting/county-parcel-audience-enroll'
...
export function CampaignAudienceWorkbench({ campaignId, campaignName, campaignKind, total, canEditAudience, onAudienceChanged }: { campaignId: string; campaignName: string; campaignKind?: 'dialer' | 'sms'; total: number; canEditAudience: boolean; onAudienceChanged?: () => void | Promise<void> }) {
  ...
  async function removeMember(member: ProspectingCampaignMember) {
    ...
    try {
      ...
      setConfirmRemovalId(null)
      setNotice(`${member.lead?.fullName || formatPhone(member.phone)} was removed. No unsent campaign actions remain.`)
  ...
  async function recipientReviewed(member: ProspectingCampaignMember, phone: string) {
    ...
    setNotice(`${member.lead?.fullName || formatPhone(phone)} now has one reviewed SMS recipient. Nothing has been sent.`)
    await onAudienceChanged?.()
  }
  ...
  return <article className="crm-panel rounded-2xl p-5 sm:p-6" aria-label="Campaign audience workbench">
    ...
    {canEditAudience ? <CountyAudienceInventory campaignId={campaignId} campaignName={campaignName} campaignKind={campaignKind} onEnrolled={onAudienceChanged} /> : null}
    {canEditAudience ? <CountyParcelAudienceEnroll campaignId={campaignId} campaignName={campaignName} campaignKind={campaignKind} onEnrolled={onAudienceChanged} /> : null}
    ...
    {!searchPending && members.length ? <div className="mt-5 overflow-hidden rounded-xl border border-[var(--crm-border)]"><div className="hidden grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_6rem_7rem_10rem] gap-3 bg-[var(--crm-surface-subtle)] px-4 py-2 text-[9px] font-black uppercase tracking-wider text-[var(--crm-text-muted)] sm:grid"><span>Seller</span><span>Property</span><span>Phones</span><span>Status</span><span>Actions</span></div>{members.map((member) => <div key={member.id} className="grid gap-2 border-t border-[var(--crm-border)] px-4 py-3 first:border-t-0 sm:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_6rem_7rem_10rem] sm:items-center"><div className="min-w-0"><p className="truncate text-sm font-black text-[var(--crm-ink)]">{member.lead?.fullName || formatPhone(member.phone)}</p><p className="text-[10px] text-[var(--crm-text-muted)]">{member.subjectKind === 'prospect' ? 'Source prospect' : 'CRM lead'} · {formatPhone(member.phone)}</p></div><p className="truncate text-xs text-[var(--crm-text-muted)]">{member.lead?.propertyAddress || 'Property not recorded'}</p><p className="text-[10px] font-bold text-[var(--crm-text-muted)]">{member.readyContactCount} ready{member.suppressedContactCount ? ` · ${member.suppressedContactCount} blocked` : ''}</p><span className={`w-fit rounded-full px-2 py-1 text-[9px] font-black uppercase ${memberTone(member)}`}>{member.suppressionReason || member.status.replace('_', ' ')}</span><div className="flex flex-wrap items-center gap-1"><Link href={`/conversations?lead=${encodeURIComponent(member.leadId || `phone:${member.phone}`)}`} className="rounded-lg px-2 py-1.5 text-[9px] font-black text-[var(--crm-brand)] hover:bg-[var(--crm-brand-soft)]" aria-label={`Open conversation with ${member.lead?.fullName || formatPhone(member.phone)}`}>Open inbox</Link>{campaignKind === 'sms' && canEditAudience && member.status !== 'removed' && member.status !== 'replied' && member.status !== 'suppressed' ? <CampaignSmsRecipientReview campaignId={campaignId} memberId={member.id} label={member.lead?.fullName || formatPhone(member.phone)} onReviewed={(phone) => recipientReviewed(member, phone)} /> : null}{canEditAudience && member.status !== 'removed' ? confirmRemovalId === member.id ? <><button type="button" onClick={() => void removeMember(member)} disabled={Boolean(removingId)} className="rounded-lg bg-[var(--crm-danger)] px-2 py-1.5 text-[9px] font-black text-white disabled:opacity-50">{removingId === member.id ? 'Removing…' : 'Confirm'}</button><button type="button" onClick={() => setConfirmRemovalId(null)} disabled={Boolean(removingId)} className="rounded-lg bg-[var(--crm-surface-subtle)] px-2 py-1.5 text-[9px] font-black text-[var(--crm-text-muted)] disabled:opacity-50">Keep</button></> : <button type="button" onClick={() => setConfirmRemovalId(member.id)} className="rounded-lg px-2 py-1.5 text-[9px] font-black text-[var(--crm-danger)] hover:bg-[var(--crm-danger-soft)]" aria-label={`Remove ${member.lead?.fullName || formatPhone(member.phone)} from campaign`}>Remove</button> : member.status === 'removed' ? <span className="text-[9px] font-bold text-[var(--crm-text-dim)]">History</span> : null}</div></div>)}</div> : null}

...
export function CampaignDeliveryPulse({ campaign }: { campaign: ProspectingCampaignDetail }) {
  ...
  return <article className="crm-panel rounded-2xl p-5 sm:p-6" aria-label="Campaign delivery pulse">
    ...
    <p className="mt-4 text-xs leading-5 text-[var(--crm-text-muted)]">{campaign.operations.lastSentAt ? `Last successful send: ${dateLabel(campaign.operations.lastSentAt)}.` : 'No successful sends have been recorded yet.'} Failed actions stop after the bounded retry policy and remain visible in Operations.</p>
  </article>
}

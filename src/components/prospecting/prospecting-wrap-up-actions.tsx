  ...
  return <>
    <section aria-label="Wrap-up and next actions" className={toolbar || tab ? 'min-w-0' : 'ck-card p-4'}>
      <div className={`${toolbar ? 'mb-2' : 'mb-3'} flex items-start justify-between gap-3`}>
        ...
        <span className={`rounded-full border px-2 py-1 text-[9px] font-black uppercase tracking-wider ${props.leadId ? 'border-[var(--crm-brand-border)] bg-[var(--crm-brand-soft)] text-[var(--crm-brand)]' : 'border-[var(--ck-border)] bg-[var(--ck-surface-elev)] text-[var(--ck-text-muted)]'}`}>{props.leadId ? 'Lead' : 'Source Prospect'}</span>
      </div>
      ...
      {props.readOnly ? <p className={`${toolbar ? 'mt-2' : 'mt-3 rounded-lg border border-[var(--ck-border)] bg-[var(--ck-surface-elev)] px-3 py-2'} text-[10px] leading-4 text-[var(--ck-text-muted)]`}>Next actions are visible but locked until this window owns the live dialing session.</p> : null}

...
function reportHref(campaignId: string | null, runNumber: number | null, page: number, range: MyDayDateRange) {
  const query = new URLSearchParams({ campaign: campaignId || 'all', range: range.preset })
  ...
  return <main className="min-h-0 flex-1 overflow-y-auto bg-[var(--crm-canvas)] p-3 sm:p-5 lg:p-7">
    <div className="mx-auto max-w-[1540px] space-y-5">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <ProspectingReportFilters key={`${report.campaign.id || 'all'}:${report.runNumber || 'all'}:${range.preset}:${range.from}:${range.to}`} campaigns={campaignOptions} campaignId={report.campaign.id} runNumber={report.runNumber} runs={report.runs} range={range} today={today} />
      </div>
      ...
      <section className="overflow-hidden rounded-3xl bg-[linear-gradient(135deg,#121a26_0%,#16243a_58%,#3a202b_100%)] p-5 text-white shadow-[0_24px_70px_rgba(5,13,25,0.2)] sm:p-7">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          ...
          {report.campaign.currentRunNumber ? <p className="text-xs font-bold text-white/55">Current campaign run: {report.campaign.currentRunNumber}</p> : <p className="text-xs font-bold text-white/55">{range.from === range.to ? range.from : `${range.from} through ${range.to}`}</p>}
        </div>
      ...
      {metrics.attempts === 0 ? <EmptyReport campaignId={report.campaign.id} /> : <>
        ...
        <section className="crm-panel overflow-hidden rounded-2xl">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[72rem] text-left text-xs">
              ...
              <tbody className="divide-y divide-[var(--crm-border)]">{report.attempts.items.map((attempt) => <tr key={attempt.id} className="hover:bg-[var(--crm-surface-hover)]"><td className="whitespace-nowrap px-5 py-4 font-bold text-[var(--crm-text-muted)]">{dateTime(attempt.startedAt || attempt.createdAt)}</td>{report.campaign.id === null ? <td className="max-w-56 px-4 py-4 font-black text-[var(--crm-ink)]">{attempt.campaignName}</td> : null}<td className="max-w-sm px-4 py-4"><p className="font-black text-[var(--crm-ink)]">{attempt.sellerName || 'Unknown seller'}</p><p className="mt-1 truncate text-[10px] text-[var(--crm-text-muted)]">{attempt.propertyAddress || 'Property not linked'}</p></td><td className="whitespace-nowrap px-4 py-4 font-mono font-black text-[var(--crm-ink)]">{formatPhone(attempt.phone)}</td><td className="px-4 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black ${resultTone(attempt.status, attempt.reached)}`}>{resultLabel(attempt.status, attempt.disposition)}</span></td><td className="whitespace-nowrap px-4 py-4 font-bold text-[var(--crm-ink)]">{attempt.agentName}</td><td className="px-4 py-4 font-black text-[var(--crm-text-muted)]">#{attempt.runNumber}</td><td className="whitespace-nowrap px-4 py-4 font-bold text-[var(--crm-text-muted)]">{duration(attempt.durationSeconds)}</td><td className="whitespace-nowrap px-5 py-4 font-mono text-[var(--crm-text-muted)]">{formatPhone(attempt.callerId)}</td></tr>)}</tbody>
            </table>
          </div>
        ...
        <section className="crm-panel overflow-hidden rounded-2xl">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[52rem] text-left text-xs"><thead className="bg-[var(--crm-surface-subtle)] text-[10px] font-black uppercase tracking-wider text-[var(--crm-text-muted)]"><tr><th className="px-5 py-3">Started</th>{report.campaign.id === null ? <th className="px-4 py-3">Campaign</th> : null}<th className="px-4 py-3">Agent</th><th className="px-4 py-3">Run</th><th className="px-4 py-3">Queue</th><th className="px-4 py-3">Results</th><th className="px-4 py-3">Reached</th><th className="px-4 py-3">Skipped</th><th className="px-5 py-3">Status</th></tr></thead><tbody className="divide-y divide-[var(--crm-border)]">{report.sessions.map((session) => <tr key={session.id}><td className="whitespace-nowrap px-5 py-4 font-bold text-[var(--crm-text-muted)]">{dateTime(session.startedAt)}</td>{report.campaign.id === null ? <td className="max-w-56 px-4 py-4 font-black text-[var(--crm-ink)]">{session.campaignName}</td> : null}<td className="px-4 py-4 font-black text-[var(--crm-ink)]">{session.agentName}</td><td className="px-4 py-4 font-black text-[var(--crm-text-muted)]">#{session.runNumber}</td><td className="px-4 py-4 font-black text-[var(--crm-ink)]">{session.queueSize}</td><td className="px-4 py-4 font-black text-[var(--crm-ink)]">{session.resultsSaved}</td><td className="px-4 py-4 font-black text-[var(--crm-ink)]">{session.reached}</td><td className="px-4 py-4 font-black text-[var(--crm-ink)]">{session.skips}</td><td className="px-5 py-4"><span className="rounded-full bg-[var(--crm-surface-subtle)] px-2.5 py-1 text-[10px] font-black uppercase text-[var(--crm-text-muted)]">{session.status}</span></td></tr>)}</tbody></table></div>
        </section>
      </>}
    </div>
  </main>

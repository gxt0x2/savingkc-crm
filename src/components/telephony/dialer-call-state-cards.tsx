...
export function ActiveCallCard({ callTimer, dialNumber, leadName, muted, onHangup, onToggleMute, status, workspace = false }: ActiveCallCardProps) {
  ...
  >
    <div className="mb-4 text-center">
      ...
      {connected ? <p className={`mt-1 font-mono text-xl font-bold ${workspace ? 'text-[var(--prospecting-success)]' : 'text-[#FF7A7A]'}`}>{callTimer}</p> : <p className={`mt-1 animate-pulse text-sm ${workspace ? 'text-[var(--prospecting-info)]' : 'text-white/80'}`}>Dialing...</p>}
    </div>
    <div className="flex gap-3">

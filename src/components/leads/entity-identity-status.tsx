  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-1.5" aria-label="Canonical CRM identity status">
      <span
        ...
        title={needsReview ? `${context.openIdentityConflicts} identity conflict${context.openIdentityConflicts === 1 ? '' : 's'} require review.` : linked ? 'This workspace reads identity, property, and opportunity fields from the canonical CRM records.' : 'The canonical record projection is incomplete.'}
      >
        <Icon name={needsReview ? 'warning' : linked ? 'verified' : 'sync'} className="text-[13px]" />
        {needsReview ? 'Identity review' : linked ? 'Canonical record' : 'Record syncing'}
      </span>

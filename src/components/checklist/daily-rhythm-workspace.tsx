  ...
  return <main className="mx-auto flex w-full max-w-[1180px] flex-col gap-4 px-4 py-5 sm:px-6 lg:py-7">
    <header className="crm-panel overflow-hidden rounded-2xl">
      <div className="grid gap-6 px-6 py-6 lg:grid-cols-[1fr_360px] lg:px-8">
        <div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[var(--crm-brand)]">Daily Rhythm</p><h1 className="mt-1 text-[32px] font-black tracking-[-0.04em]">Let’s make today count, {teamMemberName}.</h1><p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-[var(--crm-text-muted)]">Start with purpose, protect the important work, and finish with nothing left hanging.</p><div className="mt-4 inline-flex items-center gap-2 rounded-full bg-[var(--crm-brand-soft)] px-3 py-1.5 text-xs font-black text-[var(--crm-brand)]"><Icon name="bolt" className="text-[16px]" />One focused step at a time.</div><div className="mt-4 flex gap-2 text-xs font-bold"><span className="rounded-full border border-[var(--crm-border)] bg-[var(--crm-surface)] px-3 py-1.5">{teamMemberName}</span><span className="py-1.5 text-[var(--crm-text-muted)]">{dateLabel}</span></div></div>
    ...
    <section className="crm-panel overflow-hidden rounded-2xl">
      ...
      {active === 'sod' ? <div className="grid min-h-[500px] lg:grid-cols-[270px_1fr]">
        ...
        <div className="flex flex-col p-6 lg:p-8"><div className="flex-1"><p className="text-[10px] font-black uppercase tracking-widest text-[var(--crm-brand)]">{String(wizardStep + 1).padStart(2, '0')}</p><h2 className="mt-1 text-2xl font-black tracking-tight">{step.title}</h2><p className="mt-1 text-sm text-[var(--crm-text-muted)]">{step.detail}</p><div className="mt-6 max-w-2xl">
          ...
          {step.id === 'pipeline' ? <SummaryCard icon="conversion_path" value={myDay.queue?.length ?? 0} label="priority actions ready" href="/contacts?list=new" action="Open Pipeline" /> : null}

...
export function DailyRhythmWorkspace({ userEmail }: { userEmail: string }) {
  ...
  return <main className="mx-auto flex w-full max-w-[1180px] flex-col gap-4 px-4 py-5 sm:px-6 lg:py-7">
    <header className="relative overflow-hidden rounded-2xl border border-[var(--crm-border)] bg-[#e7efed] shadow-sm">
      <div aria-hidden="true" className="absolute inset-0 bg-cover" style={{ backgroundImage: "url('/kc-skyline.jpg')", backgroundPosition: 'center 66%' }} />
      <div aria-hidden="true" className="absolute inset-0" style={{ background: 'linear-gradient(90deg, rgba(249,250,249,0.97) 0%, rgba(249,250,249,0.92) 42%, rgba(236,244,242,0.62) 70%, rgba(219,235,231,0.30) 100%)' }} />
      <div aria-hidden="true" className="absolute inset-0" style={{ background: 'linear-gradient(0deg, rgba(245,248,247,0.72) 0%, rgba(245,248,247,0.08) 70%)' }} />
      <div className="relative grid gap-6 px-6 py-8 text-[#151719] lg:grid-cols-[1fr_360px] lg:px-8 lg:py-10">
        ...
        <div className="rounded-xl border border-white p-5 text-[#151719] shadow-[0_18px_55px_rgba(25,55,48,0.22)] backdrop-blur-sm" style={{ backgroundColor: 'rgba(255,255,255,0.94)' }}><div className="flex items-center justify-between"><span className="text-xs font-black uppercase text-[#60656a]">Today’s rhythm</span><span className="text-2xl font-black text-[#151719]">{completionCount}/2</span></div><div className="mt-4 grid grid-cols-2 gap-2">{(['sod', 'eod'] as Protocol[]).map((protocol) => { const done = Boolean(daily?.[protocol]); return <div key={protocol} className="rounded-lg border p-3 text-[#151719]" style={{ borderColor: done ? '#bfe4d2' : '#dfe3e5', backgroundColor: done ? '#eefaf4' : '#ffffff' }}><Icon name={done ? 'check_circle' : protocol === 'sod' ? 'light_mode' : 'dark_mode'} className={cn('text-[20px]', done ? 'text-[#11884c]' : 'text-[#62686d]')} /><p className="mt-1 text-xs font-black text-[#151719]">{protocol === 'sod' ? 'Morning Launch' : 'Daily Closeout'}</p><p className="mt-0.5 text-[10px] text-[#6b7075]">{done ? `Saved ${formatTime(daily![protocol]!.submittedAt)}` : 'Not completed'}</p></div> })}</div></div>
      </div>
    </header>
